...
# toshio-secret-key='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'
# --------------------------------------
...
# openai==1.93.0にも現時点でAgent SDKは統合されていない。
...
# ===== OpenAI Agents SDK ===== # まず、公式ドキュメント推奨の形式を試す：
try:
    from agents import (
        Agent,
        Runner,
        ...
        function_tool,                      # 使わない場合は削除可
    )
    # もし上記がエラーになる場合は、個別のモジュールからインポート：
except ImportError:
    ...
    from agents.tool import function_tool  # 使わない場合は削除可
